...
{try{$('#pin-form').submit(function(e){e.preventDefault();get_order_data($(this).data('order_id'),$(this).find('input').val());});if(location.href=='/cart/'){refresh_cart('.cart-data__inner');}
$('.js--order-form').submit(function(e){e.preventDefault();var form=$(this);if(form.data('token')!==undefined)
...
{try{var body=$('body');if(!(body.hasClass('loading')))
{body.addClass('loading');var spinner="<i class='icon-spin icon-2x animate-spin'></i>";var result_block=$('.js--order-pin');result_block.find('.order-pin__error').html(spinner);var data={action:'get_order','order_id':order_id,'pin':pin};$.post('/wp-admin/admin-ajax.php',data,function(result)
{$('.product-buy__qty div').prop('disabled',false);body.removeClass('loading');var result_json=JSON.parse(result);if(result_json.status===0)
{result_block.html(result_json.html);}
else{result_block.find('.order-pin__error').html('<p class="text-danger">'+result_json.html+'</p>');}
console.log('Cart reloaded!');result_block.find('i.animate-spin').remove();});}}
catch(e){console.log(e);}};